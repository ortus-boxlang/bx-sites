...
`docs/` がデフォルトのソースフォルダーです。`bxSites new my-site
--source=src` を使うと、ページは `src/` 以下に生成されます。その後の
コマンドはソースフォルダーを自動検出し、出力先は引き続き `site/` です。
短縮オプション `--docs` と `--site` は、それぞれ `--source=docs` と
`--source=src` のエイリアスです。
...
!!! note "docs/ または src/"
    ...
    すべてのバーブ（`build`、`serve`、`audit`、`lint`、`page:new` など）が
