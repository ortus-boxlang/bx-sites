...
superamento/fallimento (quello è compito di `audit`).
...
lato HTML compilato già risolve (`audit`), applicato invece al sorgente
...
grezzo di `docs/`, distinto da `audit` (che ispeziona solo un `site/` già
compilato). Verifica:
