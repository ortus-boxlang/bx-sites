...
不可のプレースホルダー）だけをレンダリングします - クリック時に任意の
JS を実行するという概念はありません。どこかへ
...
静的なままです - 読者が実際にクライアントサイドでソート・フィルタできるテーブルが
欲しい場合は、`| Feature | Status |` というパイプ構文を書く代わりに、Alpine に行（row）を
...
`rows` を手で書くこと自体は動作しますが、それでも JS オブジェクトリテラルの
中に生きているコンテンツであり、他の再利用可能なデータとはかけ離れた場所で
編集することになります。同じ行データが、他の場所のプレーンなテーブルや、
複数のページでも必要な場合は、[再利用可能なデータ](data-files.md) と
`$jsonAttr()` を組み合わせることで、手書きの配列の代わりに、本物の
`docs/data/*.yaml`/`.json` の内容を `x-data` に供給できます:
...
上記と同じ `x-for`/`x-model`/ソートロジックですが、ページに焼き込まれた
リテラルの代わりに `docs/data/providers.yaml` に支えられているだけです -
完全なレシピ（そして `"..."` でクォートされた属性の中に安全に収まるために、
`jsonSerialize()` だけでなく `encodeForHtmlAttribute()` が必要な理由）に
ついては、[データファイル: データを利用する](data-files.md#consuming-data)
を参照してください。
