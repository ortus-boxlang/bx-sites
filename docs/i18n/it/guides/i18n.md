...
scritta da destra a sinistra, la propria direzione) in `bxdocs.json`:
...
`docs/i18n/es/`, `bxDocs build` la recepisce da sé.
...
un menu a discesa della lingua nell'header - niente da attivare
esplicitamente, come per il [selettore di versione](../configuration.md#versioning).
Scegli una locale che non stai attualmente compilando e semplicemente non
verrà renderizzata affatto.
...
- **Versioni e locale non si combinano.** Un albero
  `docs/versions/<name>/` si compila sempre solo nella locale
  predefinita, senza un proprio selettore di lingua; un albero di locale
  non mostra mai un selettore di versione. Usare entrambe le
  funzionalità sullo stesso progetto funziona, semplicemente non si
  incrociano - vedi [Versionamento](../configuration.md#versioning).
