...
- **API reference generation for BoxLang and ColdBox** - `bxSites docbox` turns your classes into themed, searchable API pages via DocBox's JSON output, and `bxSites coldbox` documents a whole ColdBox app - routes, handlers, models and WireBox mappings, modules, interceptors, scheduled tasks - straight from its conventions, without booting it. See [DocBox API Reference](docs/guides/docbox.md) and [ColdBox Applications](docs/guides/coldbox.md).
...
| `docbox` | Generate a BoxLang/CFML API reference from [DocBox](https://docbox.ortusbooks.com) into the content dir (needs the `bx-docbox` module) |
| `coldbox` | Document a ColdBox application from its conventions: routes, handlers, models, modules, interceptors, scheduled tasks |
...
- **[bxsites.io](https://bxsites.io/)** - the latest released version (`bxsites.yaml`'s own `versions.default`)
- **[bxsites.io/next](https://bxsites.io/next/)** - unreleased docs, currently being written toward the next release
