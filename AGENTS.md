...
- **The CLI verb dispatcher's top-level `try/catch` (`ModuleConfig.bx`) swallows the real stack trace to a bare `Error: <message>`, and its returned exit code does not reliably propagate to the actual OS-level process exit status when invoked as `boxlang module:bxDocs <verb>`.** This means `set -e`/`set -euo pipefail` in a calling shell script (`buildMultiTheme.sh`) can fail to catch a genuinely crashed build - the workflow step reports **success** while a whole theme's blog build silently produced incomplete/stale output. Combined with `pages.yml`'s `keep_files: true` on the gh-pages deploy (which never deletes old files), a crashed build can leave broken content live indefinitely with a fully green CI run. **When a fix doesn't appear to take effect on the live site after a "successful" deploy, don't trust the green checkmark - pull the actual `get_job_logs` output for the "Build the site" step and grep it for `Error:`.**
- **The TestBox suite mocks the real integrations** (`FakeImageVariantGenerator`, `FakeMarkdownRenderer`, and similar fakes for the blog builder) - a green `tests` CI job does **not** prove a real `boxlang module:bxDocs build` succeeds. Only the `pages.yml` "Publish Docs" workflow (or a local `boxlang module:bxDocs build` run with the real `bx-markdown`/`bx-yaml`/`bx-esapi`/`bx-image` modules installed, not the test fakes) exercises the true build path. Verify any change to `BuildPipeline.bx`, `BlogBuilder.bx`, `ModuleConfig.bx`'s CLI dispatch, or `vendorIcons.mjs`/`vendorAssets.mjs` against a real build before trusting it's fixed - a real BoxLang install is one `curl -fsSL https://install.boxlang.io | bash` away, and ForgeBox modules can be fetched directly from `https://downloads.ortussolutions.com/ortussolutions/boxlang-modules/<module>/<version>/<module>-<version>.zip` (get the exact URL from `https://www.forgebox.io/api/v1/entry/<module>`'s `latestVersion.downloadURL`) if `box install` itself is blocked by network/cert restrictions in a sandboxed environment.
