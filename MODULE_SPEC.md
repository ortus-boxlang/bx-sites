...
| `deploy` | Build + ship `site/` to a real target: s3, azure, gcs, firebase, ftp, sftp, rsync, netlify, vercel, cloudflare-pages, local, or github-pages - via a `deployments/*.json` entry or flag-only shorthand (`local`/`github-pages`) |
