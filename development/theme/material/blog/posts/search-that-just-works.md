...
summary: A static site with no server-side search index still deserves fast, relevant search - here's how BX Docs pulls that off by default, and when to reach for something bigger.
description: How BX Docs' default lunr.js-based static search works, plus when and how to switch to Algolia DocSearch or Pagefind instead.
image: assets/blog/search-that-just-works-cover.svg
---

Search is one of those features that either quietly works or loudly doesn't, and for a static docs site it's easy to end up with the latter - no server to query, so search either gets bolted on as a third-party widget or skipped entirely. BX Docs takes the same approach [mkdocs](https://www.mkdocs.org/) does by default: fully static, fully client-side, no server or external service required.

<!-- more -->

## How the default provider works

1. At `bxDocs build` time, `SearchIndexer` walks every non-hidden page and writes `site/search-index.json` - one entry per page, with its title, URL, frontmatter tags, every heading's text, and a truncated plain-text copy of the body.
...
The `pagefind` CLI has to already be on your `PATH` - BX Docs shells out to it right after the doc tree (including every version and locale) is written, and a missing binary fails the build loudly rather than shipping a site whose search silently doesn't work.
